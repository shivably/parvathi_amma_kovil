module.exports = function(app, db) {
  ...
  app.get('/api/kovil/id/:id', (req, res) => {
    processData(res, "SELECT * FROM Kovil where id == "+req.params.id);
  });

  // Load all products: http://localhost:4300/api/income_type/
  app.get('/api/kovil', (req, res) => {
    processData(res, "SELECT * FROM Kovil");
  });
...
function insertProduct(income_type, res, db){
  ...
  var sql = `insert into Kovil (name, description) 
          VALUES 
          (?, ?);`;
...
function updateProduct(product, res, db){
  ...
  if(!id){
      ...
      var sql = `delete from Kovil where id = ?;`;
      var values = [id];
...
function modifyMemberDetails(income_type, res, db){
  ...
  if(!id){
      ...
      var sql = `update Kovil set name = ?, description = ? where id = ?;`;
